...
function withTimeout(promise, timeout) {
  if (!promise || typeof promise.then !== 'function') {
    return Promise.reject(new TypeError('withTimeout: invalid promise provided'));
  }
  if (timeout <= 0) {
    return Promise.reject(new RangeError('withTimeout: timeout must be greater than 0'));
  }
  ...
  const timeoutPromise = new Promise((_, reject) => {
    timerId = setTimeout(() => {
      reject(new Error(`withTimeout: operation timed out after ${timeout}ms`));
    }, timeout);
  });
